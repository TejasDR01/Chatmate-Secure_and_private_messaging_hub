      ...
      try {
        if(localdata){
          ...
          if(Chats.status===200){
            ...
            await setsocket(
              io.connect(SOCKET_URL).emit("join_room","chatting").on("recieve_message",(data)=>{
  ...
  return (
    <>
      <div className="User">
        ...
        <div className="name"><b>{username}</b></div>
          ...
          <div className="chatbox">
          {
            (()=>{
              ...
              for(let element of chats){
                ...
                const minutes=timestamp.getMinutes()>9?timestamp.getMinutes().toString():"0"+timestamp.getMinutes().toString();
                const time=hours>12 ?(hours-12).toString()+":"+minutes+"pm": (hours===0 ?"12": hours.toString())+":"+minutes+"am";

                if(prevdate!==date)
                  p.push(<ChatDate date={date} />);
                p.push(
                  <Chat align={element.Name===username ? "right" : "left"} name={element.Name} message={element.message} time={time}/>
                );
                prevdate=date;
              }
              return p;
            ...
            <form onSubmit={handleSubmit} className="chat-bar">
              ...
              <button
                className="send"
                onMouseEnter={(e)=>{ e.target.style.backgroundColor="#0b0d7a";}}
